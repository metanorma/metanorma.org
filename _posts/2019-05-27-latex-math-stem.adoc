...
[source,adoc]
----
// When general [stem] is AsciiMath:
...
[source,adoc]
----
// When general [stem] is TeX math:
The formula is stem:[\displaystyle{\sum_{{{i}={1}}}^{{n}}}{i}^{3}={\left(\frac{{{n}{\left({n}+{1}\right)}}}{{2}}\right)}^{2}]
...
NOTE: Pssst... we also helped https://dlmf.nist.gov/LaTeXML[LaTeXML] create

---
layout: post
title:  "Writing in Asciidoctor, Part 3"
date:   2018-12-16
categories: about
excerpt: >-
...
rigorously in order for a document to comply. This maenas that you will need to go back
...
flavours of Metanorma (like ISO), although it will still be added to document metadata
in the XML file (the `bibdata` element). In flavours where it is rendered, it may
...
ISO (la Internacia OrganizaÄo por Normigo)...
--
...
in that case _Symbols and Abbreviations_ is expected to be a distinct subclause.)
...
while _Terms and Definitions_ treats each term as a separate subclause, with its
...
* Unordered lists, which are bulleted, and can be nested. (The style of bullet varies by flavour.)
...
by the Metanorma flavour; this is so that ordered list items can be referenced consistently.
...
* Adnomitions (including Notes, and Warnings/Cautions).
...
the docuemnt attributes and markup customisations common to all of Metanorma)
* The Github page for the gem specific to your Metanorma flavour; e.g.
...
is autonumbered. Anything that can be considered a cross-reference
...
URLs are handled by a macro, consisting of a recognisable URL, followed optionally 
(without space) by square brackets containing the text to be hyperlinked;
e.g. `http://www.iso.org/[International Organization for Standardization]`.
...
to be preceded by puncutation, although it is recognised as is, and can also be delimited
...
gem.) And if you get stuck, you can also ask your friendly neighbourhood wizard,
care of info@ribose.com, or by raising an issue on GitHub on the repository of your
Metanorma flavour's gem.

.The author, trying to pass as a friendly neighbourhood wizard

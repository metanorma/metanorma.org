...
The commonly used in MPFA documents
link:/author/topics/document-format/attributes/[document attributes]
are:
...
`:copyright-year:`::
Year which will be claimed as when the copyright for
the document was issued

`:title:`::
*Mandatory.* Main component of the English title of the document
NOTE: The first line of the AsciiDoc document, which contains the title
...
`:status:`::
Document status. For example, `published` or `draft`

`:draft:`::
Signals to render reviewer notes

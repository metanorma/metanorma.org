...
`:docnumber:` The document number assigned to the OGC document (without the "OGC" prefix).
The document number is formulated following the following rules:
...
* A major version of a document receives a new document number, including likely a new year.

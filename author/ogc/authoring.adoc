...
*** Components with the `class` attribute `conditions`, `test-purpose`, or `test-method`
are extrcted and presented in that order, as _Conditions_, _Test Purpose_ and
_Test Method_, followed by the component contents.

*** Components with the `class` attribute `part` are extracted and presented in order:
each Part is rendered as an incrementing capital letter (_A_, _B_, _C_ and so on),
followed by the component contents.
...
* The remaining rows of the requirement are the remaining components of the requirement,
...
*** `[.verification]`
*** `[.import]`
*** `[.component]`, as discussed above: `[.component,class=conditions]`, `[.component,class=test-purpose]`,
`[.component,class=test-method]`, `[.component,class=part]`

...
but some authors may prefer specifying attributes in the header. However, not
all attributes are supported in the block attribute syntax (`target`, `indirect-dependency`).
...
* `identifier` (mandatory). Label of requirement, typically a URI. This must be unique
in the document (as required by ModSpec), and is also used for referencing.
Plain text. (Was `label` until https://github.com/metanorma/metanorma-ogc/releases/tag/v2.2.0 .)
...
* `target`. Requirement being tested by a Conformance test; Requirements class being tested by a
Conformance class. Mandatory for those two requirement types. Is the identifier of the
Requirement or Requirements class. Replaces `subject` in that context. 
Only supported in definition list syntax. [added in https://github.com/metanorma/metanorma-ogc/releases/tag/v2.2.0] 
...
* `target`. Associated Requirements class. Populated with the identifier of the
...
** The `indirect-dependency` attribute of the requirement, if given: _Indirect Dependency_ followed
by the attribute value. If there are multiple values of the attribute,
they are semicolon delimited.

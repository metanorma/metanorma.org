...
NOTE:: A part is distinct from a step
(as appears in <<conftest>>): a part is a component of a requirement, which is itself
a requirement. A step is a stage in a process of testing a requirement: it only
makes sense within a test method.
...
added in https://github.com/metanorma/metanorma-ogc/releases/tag/v1.4.2]. Steps can be
nested; the nested list order is arabic, then alphabetic, then roman.

...
.Unnumbering blocks by prepending `[%unnumbered]` attribute. (It works for almost any block.)
[source]
--
// Unnumbered figure
[%unnumbered]
...
Now, there is a more estetic way to apply this attribute.
...
.Unnumbering blocks by appending `%unnumbered` to its name.
[source]
--
// Unnumbered example
[example%unnumbered]
...
As a rule of thumb, if you don't remember how to unnumber a certain block, just prepend `[%unnumbered]`, it works for any block that supports unnumbering.

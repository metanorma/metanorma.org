...
Ordered lists by default follow the Metanorma numbering scheme: _a), b), c),_ then _1), 2), 3)_,
then _i), ii), iii)_, then _A), B), C)_, then _I), II), III)_.

If an ordered list is specified to be of class "steps", it will start with arabic numbers:
_1), 2), 3)_, then _a), b), c),_.

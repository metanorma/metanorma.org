...
Whether to hide page header titles on the PDF.
...
[example]
====
The header titles are the "ITUPublications", "International Telecommunication Union",
with the Sector or Bureau name on the cover page.
====

`:presentation-metadata-color-cover-itu-logo:`::
Color of the ITU logo on the PDF. Typically `#1DA0DB` (default) or 'white'
depending on the contrast with background.

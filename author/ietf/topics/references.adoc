...
to a standalone `reference/title`, with no authors or dates. (Because authors are mandatory in XML RFC
references, a dummy author "Unknown Unknown" will be provided; so if at all possible, preformatted
citations should be avoided.) 
If the citation is broken down into elements,

...
by default for IETF references. That means that the generated v3 RFC XML will still end up referencing
...
A reference not autofetched from online sources is either a preformatted citation treated as a block,
...
it is processed as with autofetched references:

[source,asciidoc]
--
[bibliography]
== Normative References
* [[[ref1,1]]] MacDonald, Ian. 1991. _The New Shostakovich._ Boston: Norteastern University Press. <1>
...
As of this writing, the following features of v3 RFC XML referencing are not supported:

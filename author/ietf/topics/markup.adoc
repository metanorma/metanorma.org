...
Metanorma AsciiDoc does not permit anchors on list items: the anchors in the following are ignored.
...
AsciiDoc also permits paragraphs and open blocks (which can contain
...
<4> v3 only: `cref/name` (not supported in v2)
...
NOTE: Typical Asciidoctor-AsciiDoc attempts to match a crossreference to a section title, if it does not find
a matching anchor ID -- Metanorma does not do that (and in newer Asciidoctor-AsciiDoc versions > `1.5.7`).
If you have a citation of a bibliographic item which is identical to a section title (e.g. you have a

...
ASCII art, source code and images; they are redundant for RFC XML v3, since
...
<1> `figure/@anchor`
<2> `figure/name`
<3> `figure/sourcecode/@anchor`
<4> `figure/sourcecode/@name`
<5> `figure/sourcecode/@type`, `figure/sourcecode/@src`, `figure/sourcecode/@markers`, `figure/sourcecode/@name`. If `src` is present, the listing is not expected to have any content: content is taken from the hyperlink in the attribute.
<6> `figure/sourcecode`
...
<3> `<iref item="indexterm" primary="true">primary index term</iref>` [added in https://github.com/metanorma/metanorma-ietf/releases/tag/v2.3.0]
<4> `<iref item="indexterm" subitem="index-subterm"/>` [added in https://github.com/metanorma/metanorma-ietf/releases/tag/v2.3.0]

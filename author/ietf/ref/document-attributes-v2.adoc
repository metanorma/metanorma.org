...
[NOTE]
====
Below are listed only document attributes that are unique to Metanorma’s processing of v2 RFC XML
IETF documents. The document attributes particular to v3 RFC XML, and used in the current version of
this gem, link:document-attributes.adoc[are given separately].

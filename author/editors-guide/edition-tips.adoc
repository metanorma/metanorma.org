...
== Exporting SVG images from Adobe Illustrator

When exporting SVG files from Adobe Illustrator for inclusion in Metanorma
documents, use the following settings to ensure compatibility and proper
rendering across different output formats.

Use this following setup to ensure proper rendering across different output
formats:

image::/assets/author/edition-tips/illustrator-export-settings.png[]
...
Without using these settings, the resulting Metanorma document may not render
the SVG images correctly across different output formats (such as HTML, PDF).

The reasons for these settings are:

* Inline styles ensure that the SVG renders correctly without relying on
external stylesheets. Metanorma may be executed in an environment where external
stylesheets (as local paths or on the Internet) may not be accessible.

* Converting fonts to outlines preserves the appearance of text, avoiding font
substitution issues. Without this setting, if the target system does not have
the same fonts installed, the text will not render as intended.

* Embedding images ensures that any raster images are included in the SVG file,
preventing missing image errors if the images are not available in the target
environment.

* Unique object IDs are necessary for proper referencing and manipulation of SVG
elements. Otherwise, ID conflicts may arise if you have more than one SVG using
the same IDs, leading to rendering issues.
...
language, or are unsure how it will be rendered, do take time to check
...
== Remove justification in list item paragraphs

Paragraphs in DOCX/PDF outputs are justified by default. Removing this setting in most cases is as simple as adding `[align=left]` before the paragraph. With list items we need to apply another approach, however. This is the way to do it:
...
The output will show a normal list item, with the text aligned to the left.


== Escaping callouts in code blocks

In some situations, it is necessary to include literal instances of `<n>`
(where `n` is any natural number) inside a code block. For example:
...
However, in its current form, this code will not render as intended.
The sequences `<1>`, `<2>`, and `<3>` will be interpreted by Metanorma as
link:https://docs.asciidoctor.org/asciidoc/latest/verbatim/callouts/[*callouts*]
because they appear at the end of the line of code, resulting in the rendered
output of `(1)`, `(2)`, and `(3)`, respectively, as shown below:
...
To prevent callout recognition and preserve the literal output, you can escape the callout
syntax by prefixing it with a backslash. This ensures the code block renders as expected:

...
`industry-connections-report`:: Document is an Industry Connnections Report document (applies only to `whitepaper`)
--
...
The sponsoring bodies, if any, are indicated through the `:sponsor:` and `:sponsor-subdivision:`
attributes. As with other author values, multiple instances of sponsoring organisations and their
subdivisions are indicated as `:sponsor_2:` and `:sponsor-subdivision_2:`, `:sponsor_3:` and `:sponsor-subdivision_3:`, etc.

The values of `:sponsor-subdivision_{i}:` in turn
...
On the front line, if it is a sponsor, "IEEE" is prefixed to the first subdivision, rather than being rendered on a separate line.

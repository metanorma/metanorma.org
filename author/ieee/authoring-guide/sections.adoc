...
The easiest way to ensure that you are following the document structure that IEEE
demands, is to use a Metanorma template.

If you author a document type that doesn't have a template, check a
https://github.com/metanorma/mn-samples-ieee[sample document] for better
...
Learn how to
link:/author/ogc/authoring-guide/bibliography[enter bibliographic entries].

NOTE: In IEEE there are two "`bibliography`" sections. The "`References`"
...
IEEE SA documents support an extra definitions section that represents
...
Please refer to the
link:/author/ogc/authoring-guide/terms-definitions[terms and definitions syntax]

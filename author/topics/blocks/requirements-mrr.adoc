...
`render`:: (optional) The rendering mode of the requirement model instance.
(default: `default`)
 [added in https://github.com/metanorma/mn-requirements/releases/tag/v0.6.0]
Legal values are:

`default`::: The default rendering of the requirement model, as defined by the
requirement model scheme.

`inline`::: A rendering of the requirement model as a single line of text, with
the components of the requirement model rendered as inline elements.

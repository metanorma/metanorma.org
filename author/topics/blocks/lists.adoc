...
It is possible to specify that nested lists be rendered as 
tables [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.3.3].
Under this rendering, each level of the nested list is given a separate column,
and each branching entry of the list is given its own row. Nested titles of sublists
are treated as column headings.
...
.2+| 1. List 1 | a. Sublist 1 a|
* A
* B
* C
2+| b. Sublist 2
3+| 2. List 2
| 3. List 3 | a. Sublist 3 a|
. i. D
. ii. E
|===
====

In order to fix the relative widths of the table columns, add the attribute
`display-directives="colgroup='...'"`, where `...` is a comma-delimited list of
column widths, just as are used in the native Asciidoc attribute `cols`:

[source,adoc]
----
[display=table,display-directives="colgroup='1,2,1'"]
----

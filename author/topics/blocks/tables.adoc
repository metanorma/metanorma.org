...
The only CSS attribute guaranteed to be supported in Metanorma for table rows and cells is
`background-color`.
...
h|1 td-style:[background-color: green] td-style:[background-color: blue] |2 tr-style:[background-color: green] |3 
|===
----

This example applies `background-color: red` to the first row, `background-color: green` to the second row,
and `background-color: green;background-color: blue` (as a combination of styles) to the first (header) cell
of the second row.

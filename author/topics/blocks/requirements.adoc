...
model rendering:: defined rendering of a model
...
A single model has a default rendering, but can be rendered in different
ways [added in https://github.com/metanorma/metanorma-standoc/releases/tag/3.4.1].

* Metanorma MRR can have `default` rendering, or `inline` rendering.
...
=== Specifying model rendering

MRR and Modspec both have a default rendering. MRR has the alternate rendering
`inline` [added in https://github.com/metanorma/mn-requirements/releases/tag/v0.6.0].
This can be specified in two ways:

. Document-wide specification. Use the `:requirements-render:` document attribute
to specify the requirements rendering for the current document.
+
[example]
If you want the requirements in an ISO document to be rendered as `inline`, add
`requirements-render: inline` as a document attribute to the document.

. Instance specification. Use the `render` attribute to specify the rendering
specific to that requirement. This is extra effort, and only makes sense if you
will use more than one model within a document.
...
[source,adoc]
----
[{type-of-requirement},{model-scheme},{model-rendering}]
====
[%metadata]
...
* `{rendering}` is the name of the model rendering. This defaults to the default
rendering of the model scheme.
...
[example]
.Example of an MRR requirement with complex values
======
[source,asciidoc]
----
[requirement,model=default,render=inline]
====
[%metadata]
type:: class
...
`render`:: The requirement model rendering that applies to the instance.
 [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.4.1]

...
Subfigures are entered by including multiple images in an example block. If this markup is used,
the images will be rendered one vertically after another.
...
It is also possible to arrange the subfigures of a figure as a table; this allows both subfigures
to be aligned horizontally, and as a table with multiple rows and/or columns, possibly with a subfigure
spanning multiple rows or columns [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.1.9].
In order to do so, the table containing the subfigures needs to be embedded in an example block marked as `[figure]`.

[example]
.Figure with 3 subfigures, in tabular format
=====
[source,asciidoc]
--
[figure]

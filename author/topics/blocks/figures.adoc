...
non-textual material so that they are independently referable, and are considered
to be referenceable document blocks.
Images can also be used as elements appearing inline within a block, such as a paragraph.
...
of the `image` command. This page discusses only the markup specific to figures as blocks,
using the image macro `image::[]`.

=== Block markup

An image block macro in isolation is treated as a figure:

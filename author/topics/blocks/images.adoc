...
`narrow`::: PDF-specific. Specifies that the image will stay within a text
column.
 [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.8.6]
+
NOTE: This feature is not supported in typical AsciiDoc.
...
The values `full-page-width`, `narrow`, and `text-width` are used in PDF to
...
.Setting `width=narrow` for an image in PDF
====
[source,asciidoc]
----
[width=narrow]
...
.Setting `width=narrow` for a figure in PDF
=====
[source,asciidoc]
----
[.figure,width=narrow]

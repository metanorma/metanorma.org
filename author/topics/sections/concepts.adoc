...
However, a subclause within a terms collection, intended to be an empty terms collection itself, needs special markup, just as a
nonterm clause does, in order not to be processed as a single term. Mirroring `[.nonterm]`, the markup for an empty terms
subcollection is `[.terms]` [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.4.5].

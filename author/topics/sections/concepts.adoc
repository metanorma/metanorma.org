...
.Rendering of overridden predefined text
____
*3. Terms and definitions*
...
clause, and any descendant of a `[.terms]` clause is by default a term (see below).
...
A top-level empty terms collection, with no term subclauses inside it, will be
recognised by Metanorma as such by default:
...
However, a subclause within a terms collection, intended to be an empty terms
collection itself, needs special markup, just as a `nonterm` clause does, in
order not to be processed as a single term.

To declare a subclause as an empty terms collection, the style attribute
`[.terms]` is used.

NOTE: The usage of `[.terms]` is intended to mirror `[.nonterm]` [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.4.5].

.Example encoding of an empty terms collection
[example]
======
[source,asciidoc]
----
...
[.terms]
=== I am an empty terms collection
----
======

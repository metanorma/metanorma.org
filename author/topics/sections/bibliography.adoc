...
Predefined text is provided only for _normative references_: a bibliography
section with `normative=true`. That is the default for a section titled
"Normative references"; any other `[bibliography]` section (for example
`== Bibliography`) defaults to `normative=false` and is _informative_.

In a normative references clause, any content before the list of references
is ignored, and replaced by predefined text specific to the Metanorma flavour.

An informative bibliography receives _no_ predefined text, and the override and
`(default)` mechanisms described below do not apply to it. Where a document has
both a normative references clause and an informative bibliography, only the
normative references clause receives predefined text.

NOTE: Predefined text (and boilerplate) for informative bibliographies is not
currently supported.
...
NOTE: `(default)` is resolved only within normative references. In an
informative bibliography it is not substituted, and renders literally.
...
A bibliographic reference can optionally be followed by one or more notes. This
allows an annotated bibliography to be generated. Notes are sorted together with
their preceding bibliographic entry, and are rendered as part of that entry.
...
There are several ways of entering bibliographic references as shown in
<<entering-refs>>.
...
If no document identifier is to be provided for the reference, give the
reference tag as a space:

[example]
.Example of specifying a blank numeric reference tag
====
[source,asciidoc]
--
...
[[annotated-spans]]
=== Encoding semantic bibliographic elements ("bibliographic spans")

==== General

Metanorma supports semantic markup of bibliographic entries, to allow
generation of machine-readable references from pre-formatted reference entries.

Different from a pre-formatted reference, of which the entire content is treated
as a single block of text to be rendered as-is, with no further processing,
semantic markup allows authors to mark up segments of a reference semantically
to generate a machine-readable reference that suits the flavor's citation style.

A pre-formatted reference like the following is human-readable, but does not
indicate to Metanorma how to extract useful information from it -- what the
document identifier is, who the authors are, who the editors are, etc.

[example]
.Example encoding of a pre-formatted bibliographic item
====
[source,asciidoc]
----
Wozniak, S., Jobs, Steve, & Hoover, J. Edgar. 1991 [1996]. _Work_ In Gates W. H. &
UNICEF (eds.), Collected Essays. _Bibiographers Anonymous_, 5: 2–21. ISO 1234.
Geneva: International Standardization Organization. http://www.example.com
----
====

Metanorma provides the "bibliographic span" mechanism to allow authors to
annotate critical bibliographic information as the content of the list item
that declares the bibliographic entry. [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.1.6].

This method allows authors to provide bibliographic information at a level of
detail supported by the Relaton citation model. The span categories supported
are described below.

In the resulting reference text, any punctuation or formatting will be provided
by Metanorma, following the flavor's reference style.
...
[source,asciidoc]
----
* [[[anchor,reference tag]]],
  span:{span-type}[{span content}]
  span:{span-type}[{span content}]
  ...
  span:{span-type}[{span content}]
----
...
Metanorma supports annotations surrounding the bibliographic spans in the
reference.

While Metanorma only processes the content within the spans, authors can
choose to add punctuation, formatting and other text around the spans for
readability.
...
With no punctuation or formatting:
...
* [[[A, B]]],
  ...
  span:date[1996]
  span:title[Work]
  span:in_surname.editor[Gates] span:in_initials.editor[W. H]
...
With punctuation and formatting:
...
Note the distinction made in the example between Wozniak and Jobs (authors of
the paper), and Gates and UNICEF (editors of the book including the paper).
Similarly, note the distinction between the title of the paper (_Work_), and the
title of the book including the paper (_Collected Essays_).
====


==== Supported bibliographic span types

The span types supported are not an exhaustive list of bibliographic fields. If
more detail is required (or a dynamically generated reference is to be
generated), one of the following explicit bibliographic entry methods should be
used instead.

The span categories supported are:

`surname`::
Author surname. Must *always* precede the initials or the given name of an author.
...
`in_title`::
Title of containing bibliographic item. For types `inbook`, `inproceedings`,
`incollection`, the title of the book, proceedings, collection containing the
item.
...
`docid.XXX`::
Document identifier, of type `XXX`. The identifiers follow Relaton XML practice,
and the following types are in place for the major classes of bibliographic
identifier:
+
`metanorma-ordinal`::: ordinal number provided for reference, normally supplied
automatically by Metanorma and used only for bibliography
`title`::: identifier repeating the title of the reference, supplied automatically
in normative reference contexts when no other identifier has been provided
`metanorma`::: identifier not otherwise recognised as specific to an SDO
`repository`::: link to other document in a Metanorma collection
`ISBN`::: This identifier is an International Standard Book Number (ISBN-10 or ISBN-13)
`ISSN`::: This identifier is an International Standard Serial Number (ISSN-8 or ISSN-13)
`DOI`::: This identifier is a Digital Object Identifier (DOI)
`URN`::: This identifier is a Uniform Resource Name (`urn:` namespace)
`ISO`::: This identifier is an International Organization for Standardization publication identifier
`IEC`::: This identifier is an International Electrotechnical Commission publication identifier
`IETF`::: This identifier is an Internet Engineering Task Force publication identifier
`IEEE`::: This identifier is an Institute of Electrical and Electronics Engineers publication identifier
`BIPM`::: This identifier is a Bureau International des Poids et Mesures publication identifier
`BSI`::: This identifier is a British Standards Institution publication identifier
`NIST`::: This identifier is a National Institute of Standards and Technology publication identifier
`JIS`::: This identifier is a Japanese Industrial Standards publication identifier
`ITU`::: This identifier is an International Telecommunication Union publication identifier
`IHO`::: This identifier is an International Hydrographic Organization publication identifier
`CalConnect`::: This identifier is a CalConnect publication identifier
`PLATEAU`::: This identifier is a PLATEAU publication identifier
`Ribose`::: This identifier is a Ribose publication identifier
...
`note.XXX`::
Note, of a specialized type _XXX_. More than one type can be applied to a note;
in that case, the different values are comma delimited.
+
More than one note can be present in a bibliographic entry. [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.3.0]
+
--
For notes that are intended to be rendered with bibliographic entries, as part
of an annotated bibliography, the note type `display` is required.

[example]
====
[source,adoc]
----
* [[[A,1]]] span:note.display,editorial[This entry may be revised later.]
* [[[B,1]]] span:note.display[This entry may be revised later.]
----
====
--
...
`keyword`::
Keyword of bibliographic resource [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.1.9]
...
`language`:: language of citation (ISO 639-2 encoding) [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.2.0].
Used for localisation and internationalisaton of citation rendering.

`script`:: script of citation (ISO 15924 encoding) [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.2.0].
Used for localisation and internationalisaton of citation rendering.

`locale`:: locale of citation (encoded per IETF RFC 5646, Clause 2.2.4) [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.2.0].
Used for localisation and internationalisaton of citation rendering.

`type`::
Document type. List of valid document types given in
https://www.relaton.org/model/bibtype/[Relaton model -- Bibitem type].
The type is suppressed from rendering. If not supplied, it is assumed to be `standard`.
It is vital to provide this field, if the reference is not a standard: most bibliographic
fields are ignored when rendering standards.
+
[example]
...
=== Auto-fetch bibliographic date cut-off

NOTE: Feature available since [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.3.1].

When a document is published, it is published with a bibliography that reflects
the state of the bibliography at the time of publication. This means that if a
reference is cited with no publication date, in a document with a publication
date of 2019, its bibliography should represent the status of references as of
2019, which is what the document would have been published with.

Metanorma supports this principle of bibliographic date cut-off, to prevent
anachronism when compiling older documents.

This feature is achieved by configuring Relaton to fetch the version of the
reference as of the document date of publication or last update, if the
reference is cited with no publication date.

This means that if a reference was published before 2019, but entered into draft
in 2022, the version of the reference fetched for the 2019 document should
present the cited document as published as of 2019, not as draft as of 2022.


[[emebd-citation]]
=== Overriding auto-fetched reference fields

Depending on the quality of the bibliographic record fetched, it may be
necessary to amend or emend the auto-fetched information.

Auto-fetched references can be altered by specifying the `amend(...)` function
on the document identifier of the reference, and then providing amended
information fields using the bibliographic span notation (`span:...[]`) to
override the auto-fetched information.
 [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.1.3].

If the `amend(...)` function is not used, the bibliographic spans will be
treated as a full override: the auto-fetched information will be ignored, and
only the information in the bibliographic spans will be used for the reference.

The bibliographic span notation (`span:...[]`) is used to specify
bibliographic fields to a bibliographic record in the
bibliography [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.3.4].

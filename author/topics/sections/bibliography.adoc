...
and are rendered as part of that entry.
...
`note.XXX`::
Note, of type _XXX_. More than one type can be applied to a note; in that case, the different values
are comma delimited. 
More than one note can be present in a bibliographic entry. [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.2.5]
+
--
NOTE: In order to ensure that the note is rendered in the bibliography, as part of an annotated
bibliography, you need to include the type `display` in the note types.

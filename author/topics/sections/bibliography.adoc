...
A pre-formatted reference is not explicitly broken down into its semantic
components, but it provides a useful means of providing that information to Metanorma,
in broken down format after all: mark up segments of a reference
semantically, as data used to generate a machine-readable reference. So while the reference
looks like a human-readable reference, the markup highlights the critical information
needed to generate a citation, to the same level of detail as would be fetched by Relaton
from a bibliographic database.

So a citation like the following is human-readable, but does not indicate to Metanorma
how to extract useful information from it—what the document identifier is, who the authors
are, who the editors are, etc.
...
To address this need, a limited number of span categories can be used to annotate a
pre-formatted
reference [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.1.6].
When this is done, *only* the content in spans is processed; any additional information,
including formatting and punctuation, is ignored, though authors may choose to leave it in for
readability:
...
Note the distinction made in the example between Wozniak and Jobs (authors of the paper),
...
As punctuation and formatting are ignored, this is processed identically to the following:

[example]
.Example encoding of a bibliographic item inline with semantic markup, with no formatting or
punctuation.
====
[source,asciidoc]
----
...
Any punctuation or formatting will be provided by Metanorma, following the SDO's
reference style sheet.

The span types supported are not an exhaustive list of bibliographic fields. If more detail is
...
`docid.XXX`::
Document identifier, of type _XXX_. The identifiers follow Relaton XML practice, and the following 
types are in place for the major classes of bibliographic identifier:
+
* metanorma-ordinal: ordinal number provided for reference, normally supplied automatically by Metanorma
and used only for bibliography
* title: identifier repeating the title of the reference, supplied automatically in normative reference contexts
when no other identifier has been provided

...
* `date`: Date published. (All dates must be in YYYY-MM-DD format, as with the rest of Metanorma input.)
...
[IMPORTANT]
--
If your flavour requires date last accessed to be supplied for references with URLs,
you should use `date.accessed[....]` to do so (e.g. append `span:date.accessed[2020-01-01]`
at the end of the reference.) If you do not do so, Metanorma will try to access the URLs
in the reference when it styles them to align to the flavour's bibliographical stylesheet
(using the `relaton-render` gem). If the URI is not accessible, no date last accessed will
be given. However, SDO expectationis that authors should provide dates last accessed 

...
==== Numeric reference tag fetched automatically

When a reference tag is set to be named by default, we can force it
to be numeric by using the same previous method with the sole
difference of putting a number instead of a name.

This approach is useful we are working with flavors whose reference system
is named by default, like ITU for example.
...
NOTE: Again, any number can be entered between the parentheses.
The references will be incrementally re-sorted during compilation.

NOTE: In the case of ISO references, this practice is not necessary given
that the reference system is numeric by default. So, we can use respective
fetching identifiers without the need of numeric indication.

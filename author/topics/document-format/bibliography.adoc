...
For these types, almost all require an identifier of the specific identifier to make sense;
e.g. `note 1`, `page 77-99`, `annex A`. `whole`, `title` are not expected to take an identifier.

Any localities not in that list can only be recognised if prefixed with `locality:` <<custom-locality>>.

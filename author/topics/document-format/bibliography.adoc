...
====
* [[[h760,(1)ITU-T H.760]]] Recommendation ITU-T H.760 (2009_),__Overview of multimedia application frameworks for IPTV services_.
...
For these types, almost all require an identifier of the specific identifier to
make sense.

[example]
====
e.g. `note 1`, `page 77-99`, `annex A`
====

The locality types of `whole` and `title` are not expected to take an
identifier.

Any localities not in that list can only be recognised if prefixed with
`locality:`, see <<custom-locality>>.
...
Metanorma recognises only a fixed list of locality types in the AsciiDoc encoding of references,
as listed in <<locality-types,Locality types>>.
This list is not exhaustive of all possible locality types. For example, Metanorma is configured to
recognise "annex" as a generic reference to annex/appendixes in documents. It is not configured
to recognise appendixes instead of annexes, or (as is the case in ISO) as distinct from annexes.

The locality types not foreseen in  <<locality-types,Locality types>> are custom localities. Such a
custom locality can be entered by prefixing it with the string `locality:`.
...
[source,asciidoc]
----
// renders as: "ISO/IEC DIR 2, Annex SL, Appendix 2, Clause 3.2"

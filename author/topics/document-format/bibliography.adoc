...
* <<autofetch>>;
* <<preformatted-citations>>; and
* <<asciibib>>.

The basic form of a reference is a "pre-formatted reference", which relies on
the user to supply a formatted reference. Details of that format are available
at <<preformatted-citation>>.

It is strongly recommended that <<auto-fetch,automatic fetching>> methods be
used for data consistency and ease of use whenever possible (especially for
references to standards).

If automatic fetching is not available for the reference, and if
machine-readability or accurate rendering is important, <<asciibib>> is the best
method for entering structured and detailed bibliographic information within a
document.
...
=== Annotating pre-formatted citations semantically

While a pre-formatted citation is not explictly broken down into its semantic components,
it may be expedient to mark up segments of the citation semantically, to provide information
which is useful for parsing; for example, indicating the author surnames and publication date,
in order to generate an author-date citation.

For that reason, a limited number of span categories can be used to annotate a pre-formatted
citation [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.1.6].
These are not an exhaustive list of bibliographic fields, and if more detail is required
(or a dynamically generated citation is to be generated), one of the following explicit
bibliographic entry methods should be used instead.

The span categories supported are:

* `surname`: Author (or editor) surname.
* `initials`: Author (or editor) initials.
* `givenname`: Author (or editor) given name.
* `title`: Title.
...
[source,asciidoc]
----
* [[[A, B]]], s:surname[Wozniak], s:initials[S.] & s:givenname[Steve] s:surname[Jobs]. s:pubyear[1996]. s:title[_Work_].
s:docid.ISO[ISO 1234]. s:pubplace[Geneva]: s:publisher[International Standardization Organization].
----
...
<<locality-types>>).
This list is not exhaustive of all possible locality types.

For example, "annex" is recognized as a generic reference to annexes in
documents. It does not recognize appendixes (instead of annexes), or as distinct
from annexes (as is the case in ISO).

Locality types not listed in <<locality-types>> are entered using the
"custom localities" functionality.

Custom localities are entered by prefixing the locality type with the string
`locality:`. The locality type will be rendered as text preceding the equal
sign. The locality type is meant to be valid for all languages, and is localised
as with other localities through inclusion in the internationalisation YAML file for that language.
(So `locality:appendix` would be used for both English and French texts; the
realisation of `locality:appendix` as French _Appendice_ is a matter of gem configuration.)
...
Localities with the `locality:` prefix can be recognized in internationalization
configuration files, but they need to be added as part of the flavour..

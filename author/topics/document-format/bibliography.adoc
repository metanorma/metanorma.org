...
== Citation processing flags

Various processing flags can be entered on the document identifier, to specify different
behaviour. All such flags are optional, and need to be entered in the following order:

[source,adoc]
----
 [[[{anchor},nofetch(hidden(dropid(repo(path({document identifier or reference tag})))))]]]
----

* For `nofetch()`, see link:/author/topics/building/reference-lookup/#nofetch[Automatic reference lookup: Disabling automatic lookup]
* For `hidden()`, see <<hidden-citations>>
* For `dropid()`, see <<dropid>>
* For `repo()`, see  <<other-databases>>
* For `path()` (_mutually exclusive with ``repo()``_), see <<hyperlink-biblio>>
...
The NIST flavour of Metanorma currently supports rendering of generic
references, on an experimental basis.
...
[[dropid]]
=== Ignore document identifier

Normally the document identifier is critical to formulating both citations and bibliographies in Metanorma.
There are times however where the supplied document identifier is to be ignored in bibliographies.
For example, Metanorma builds a reference entry for a reference without auto-fetching or bibliographic detail
as the text following the document identifier, followed by the document identifier in brackets:
...
If we want to suppress inclusion of the document identifier in the bibliography (e.g. because the bibliographic
item is not a standard, so the identifier should not be used, wrap it in 
`dropid()` [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.1.4].
...
=== Entering entries using AsciiBib
...
=== Entering entries using Relaton XML

...
NOTE: anchor IDs of any type (cross-references, items, bibliographies, etc) must not contain colons, white spaces or words starting with numbers. This because the markup will be converted to XML, and such cases are not supported in that format. The permitted characters are specified by the link:https://www.w3.org/TR/xml-names11/#NT-NCName[NCName attribute for Namesapece Declaration].

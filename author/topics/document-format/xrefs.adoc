...
If the cross-reference is given with `style=basic%`, `style=short%`, or `style=full%` as its text,
and is referencing a clause, then the automatically populated cross-reference is modified from
the Metanorma default [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.2.6]:

* `short` means that the clause type and number is used as the cross-reference, as is the Metanorma default, e.g. "Clause 3.1.2";
* `basic` means that the title of the clause is used as the cross-reference, e.g. "Other considerations";
* `full` combines the title with the clause type/number cross-reference, e.g. "Clause 3.1.2, Other considerations".

In addition,

* a cross reference with `style=id%` displays the target anchor identifier (or any identifier aliased to 
the anchor) [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.2.7]

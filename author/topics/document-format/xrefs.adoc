...
If any index terms are present in a document, and the current flavour supports indexes,
then an index section will be automatically generated and appended to the end of the document.
In order to override the title of the index section, or where it will be placed, use the
index section markup provided for by Asciidoctor; any index will be appended after any content
you may choose to place in the index section, but indexes typically appear with no preface.
...
indexed automatically in postprocessing: the document does not need to include
explicit index terms for them [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v1.11.3].

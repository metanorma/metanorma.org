...
Other objects will generate a placeholder text, `[[{ID}]]`, if no text has been supplied for
the cross-reference.
...
NOTE: Paragraphs are automatically numbered for cross-referencing, although numbered paragraphs are not
displayed in any current flavor. (They were numbered in the UNECE flavor, but that flavor has been archived.)
So a paragraph may be referenced as "Clause  3, Paragraph 5", but the number "5" will not appear before the paragraph
in the currently supported flavors.

NOTE: The only cross-references possible at inline-level are to bookmarks (e.g. `[[anchor]]`).
The automatically generated text in that case is to the nearest parent with associated cross-reference text; 
typically, that will be a numbered paragraph.

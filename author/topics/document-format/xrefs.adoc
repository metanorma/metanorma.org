...
=== Manual compilation, clause

As an alternative to the foregoing, a manual table of contents can also be provided
as a clause of type `toc` [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v1.10.7].
Any table of contents entries within that manual table of contents
are expected to be provided as cross-references within unordered lists. The table of contents
can contain subclauses, each with its own list of cross-references, and other text:
...
titles of type `toc`, generated through either of the methods above,
are used instead of the title as the entry text for a
manual table of contents. (Of course, if an explicit cross-reference text is given
in a `toc` clause, that takes priority.)

...
NOTE: Metanorma AsciiDoc ignores the `start` attribute
on ordered lists. 

NOTE: Metanorma AsciiDoc no longer ignores the `type` attribute for ordered lists
used in Asciidoctor AsciiDoc [added in https://github.com/metanorma/isodoc/releases/tag/v1.3.0],
which allows the user to specify the label of an ordered list. However, the attribute
will be ignored in Word rendering, and a single label setting used instead;
by default, the  ISO/IEC DIR 2 labelling used instead.

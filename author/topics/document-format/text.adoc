...
The `identifier`` command, used to indicate that its contents are an identifier as
...
The span macro, `span:category[text]`, can be used to introduce semantic markup into 
Metanorma text [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.1.6]:
the _text_ it marks up is tagged as belonging to _category_. The text is not normally rendered any
different to normal, although the semantic markup introduced can be used to influence rendering.
...
But for the time being, the existent workaround for such cases is
the use of entity codes, more specifically, `&&#x200c;ndash;`.

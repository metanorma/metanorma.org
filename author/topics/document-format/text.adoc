...
NOTE: Only Metanorma IETF XML RFC output renders index terms.
...
Table columns can also have their widths set, as is supposrted in AsciiDoc [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v1.5.3]. Unlike native Asciidoc, Metanorma Asciidoc requires the column widths to be enumerated explicitly to generate column widths: `[cols="3"]` will not be processed as generating equal width columns, but `[cols="1,1,1"]` will.

...
There are circumstances when autonumbering of assets needs to be overriden; 
e.g. numbering out of sequence in updated documents. To address such use cases, 
assets can have an optional `number`
attribute [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v1.4.1],
giving a required asset number to override autonumbering. Any assets subsequent 
to that asset will be autonumbered so as to follow the previous asset, so long as
the supplied number belongs to the same sequence. 

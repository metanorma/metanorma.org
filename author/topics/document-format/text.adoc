...
- In MS Word, each list entry must be a single paragraph.
  if the AsciiDoc contains more than one paragraph for a list item,
...
In several standards, pseudocode is required to be entered. Unlike sourcecode,
pseudocode is typically in a proportional font, but it still needs to be indented
to reflect code structure. Moreover, pseudocode typically requires source code highlighting
...
with a pseudocode block macro. Text within a pseudocode block is treated as normal
text, including respect for inline formatting; lines need to separated by line breaks
(`+`). However, indentation spaces at the start of each line are preserved, by converting
...
[smallcap]#ExclusiveAccess# ≡ +
  *if* _ag.mode_ = _exclusive_ ∧ ∀t ∊ [smallcap]#Token# : _t.available_ *then* +
    *do forall* _t_ : ∊  [smallcap]#_Token_# +
      _t.owner_ := _ag_ +
    *enddo* +
  *endif*
====
...
Autonumbering numbers assets consecutively. Some times, more than one level of numbering

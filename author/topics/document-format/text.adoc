...
Metanorma supports index entries with primary, secondary and tertiary index terms
entered with the following
syntax. [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v1.3.10].
...
Metanorma does support formatting in index terms [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v1.7.0],
although formatting is ignored in IETF rendering.

[source,asciidoc]
--
signifying by divine providence that I, ((*Arthur*)),
was to carry Excalibur (((Sword~A~, stem:[sqrt(2)], Excalibur))).
--
...
using the `index` macro. The macro has at least two parameters, the primary index term to be crossreferenced,
and the target of the crossreference; the secondary and tertiary index term to be crossreferenced are
optional parameters.
...
Index term links are only rendered in certain flavours, and do not appear otherwise in
DOC, PDF or HTML output.

NOTE: Only Metanorma IETF XML RFC and Metanorma BIPM output renders index terms.
...
NOTE: In typical AsciiDoc, `[cols="3"]` is considered a shorthand to `[cols="1,1,1"]`, but this is not supported in Metanorma AsciiDoc.
...
is necessary for crossreferences between documents in a Metanorma document collection.

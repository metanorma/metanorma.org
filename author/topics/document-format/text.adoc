...
* lines need to separated by line breaks (`+`);
...
Cross-references are realised in Asciidoctor by assigning an anchor to the block to be referenced,
and writing a cross-reference containing that anchor ID:
...
The anchor ID follows the https://www.w3.org/TR/xml-id/#processing[NCName constraints of xml:id]:

...
As elsewhere in Metanorma, the caption of an image (in Metanorma, of the figure containing the image)
is given with a line prefixed with dot. In Asciidoctor, the title attribute is treated as identical
to the dot-prefixed caption:
...
Metanorma supports a title attribute on images for accessibility, which is distinct from the figure
caption. This is entered in Metanorma as a `titleattr` attribute:

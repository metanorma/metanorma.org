...
Admonitions (Note, Important, Warning etc.) can exceptionally be moved to the start of the document
body, before the main sequence of clauses, by giving them the option attribute 
`%beforeclauses` [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v1.3.30].
That means that the admonition(s) are considered to apply to the entire document.

[source,asciidoc]
----
[IMPORTANT,%beforeclauses]
====

...
Metanorma Asciidoctor inherits the https://docs.asciidoctor.org/asciidoc/latest/subs/replacements/[character substitutions] 
of Asciidoctor:
...
|`(R)`  | (R)
|`(TM)` | (TM)
|` - `  | &#8212; | "Only replaced if between two word characters, between a word character and a line boundary, or flanked by spaces.  When flanked by space characters (e.g., `a -- b`), the normal spaces are replaced by thin spaces (`&#8201;`)."
|`...`  | ...
|`->`   | ->
|`=>`   | =>
|`<-`   | <-
...
// `--` is rendered as an en-dash (&#8211;), which is not catered for by Asciidoctor escapes.

Like Asciidoctor, Metanorma Asciidoctor recognises HTML and XML character references, and decimal and hexadecimal Unicode code points.

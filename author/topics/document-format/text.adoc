...
[NOTE]
====
In regard to equation arrays, `latexmlmath` struggles to handle alignments. 
In consequence, the usage of environments like `eqnarray` is declined in Metanorma. 
For these cases, an uncomfortable but necessary workaround would be placing the equations in separate blocks concatenated with `+`. For example, the following equations
...
As a curious note, some link:https://www.tug.org/pracjourn/2006-4/madsen/madsen.pdf[LaTeX users] 
do not recommend using `eqnarray` in LaTeX environment neither, 
basically due to inconsistencies in vertical alignment and other things.
====

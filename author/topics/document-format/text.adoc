...
As with normal Asciidoctor, a top-level clause with style attribute `[index]` can be used
as a placeholder for an index in the rendered 
...
It is possible for there to be more than one index in a document; if so, they will 
be differentiated with a type attribute. However, this has not yet been implemented in any
Metanorma flavours.

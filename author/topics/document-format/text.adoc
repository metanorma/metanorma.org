...
* it may start with a letter, an underscore, or any other character specified by the attribute link:https://www.w3.org/TR/xml-names11/#NT-NCNameStartChar[NCNameStartChar]. It must not contain colons, white spaces, or words starting with numbers.
The permitted characters are specified by the link:https://www.w3.org/TR/xml-names11/#NT-NCName[NCName attribute for Namesapece Declaration].

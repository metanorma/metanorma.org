...
Now, the `[.nonterm]` attribute must only be used in subclauses that don't contain any
terms underneath (like the example above), otherwise, these won't be processed following the 
corresponding formatting rules. For instance, let us apply the `[.nonterm]` attribute 
to a term-containing subclause to see what happens:
...
.Incorrect rendering of a term subclause. The `[.nonterm]` attribute must be removed in this case.
image::/assets/author/topics/document-format/section-terms/fig-term-clause-incorrect.png[]


And this is not the correct rendering. The `[.nonterm]` attribute must be removed:

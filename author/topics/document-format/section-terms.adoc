...
If there are no terms and definitions defined in the document,
...
* The title of the predefined text clause will be stripped (so you could equally use `=== {blank}`);
...
We have already seen one exception to this, in `[.boilerplate]` clauses. More generally,
an introductory section can be treated as a subclause instead of a term (and will retain its
...
The syntax for citing terms has been overhauled; the following describes
the current syntax
...
// if no explicit term identifier is given, and the term is inferred through automatic term section anchors
{{term}}
term:[term] // synonym

// if no explicit term identifier is given, and the term is inferred through automatic term section anchors, and the display text differs from the cited term
{{term,text}}
term:[term,text] // synonym
----
...
* The term cited, in its canonical form;
* The text to be displayed, if it is distinct from the cited term.

Instances of cited terms are converted into a distinct `concept` element in Metanorma XML,
which includes a cross-reference to the term definition, and optionally the canonical form
of the term name.
...
term name, and optionally the text to be displayed. The anchor must be given
in angle brackets, like a normal cross-reference in AsciiDoc.
...
These need to follow the angle brackets around
the bibliographic-anchor, and precede the term and/or text.

The syntax is:

[source,adoc]
----
{{<<bibliographic-anchor>>,locality1=X,term}} (e.g. {{<<iso639-1>>,clause=3.1,language}} )

// if the display text differs from the cited term
{{<<bibliographic-anchor>>,locality1=X,text,term}}
----
...
termbase be included in the bibliography. The angle brackets should NOT
be given for any such references.
...
It is possible to cite a term defined within a document, without using an explicit term anchor.
This relies on the fact that Metanorma automatically creates anchor references for every term defined;
Metanorma can use this to look up the text of the term as it appears in the term definition.
...
system of ordered marks which can be attributed to {{instant, instants}} on the {{time axis}}, one instant being chosen as the origin

// equivalent:

system of ordered marks which can be attributed to term:[instant, instants] on the term:[time axis], one instant being chosen as the origin
----

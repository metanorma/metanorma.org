...
.Inclusion of a non-term subclause into the Terms and Definitions section using the [.nonterm] attribute.
image::/assets/author/topics/document-format/section-terms/fig-terms-non-clause-section.png[]

NOTE: please allow us to stress that the `[.nonterm]` attribute is for _exceptional_ cases only (e.g. an introduction section as shown above, or any other subclause not being a term). It should not be applied to subclauses containing terms underneath, as these will not be processed following the corresponding formatting rules.

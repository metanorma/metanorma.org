...
was published. As discussed below, Metanorma automatically converts citations of
IEV (via References) to citations of the appropriate IEC 60050 standards.
...
In the foregoing, we use `ievtermbank` as the anchor for IEV; any anchor can be used, so long
as it is mapped to the IEV reference code in the bibliography.

...
There may be link:https://www.iso.org/obp/ui/#iso:std:iso-iec:2382:ed-1:v1:en[cases]
where terminological clauses don't include any character separator like dashes or
floating points (as they usually do).
For example,
...
here, `2121372` is a valid terminological entry but it will be unwantedly referred as a top-level clause if we don't specify additional markup. So, in such cases we can write:
...
where `droploc%` serve as a flag to give just the number of the term.

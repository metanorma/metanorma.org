...
and to the IEC Electropedia). Any paragraphs or lists in the AsciiDoc input (which can replicate the expected boilerplate)
are stripped in the intermediate XML format. So:
...
under the ISO flavour has the text between the title and the first term definition overwritten
with the boilerplate required by ISO:

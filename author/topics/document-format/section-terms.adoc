...
Instances of terms in the body of the document can be marked up as such, using the `concept` 
macro [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v1.3.14].
...
If the term is defined within the current document, the concept macro gives the
anchor of the term definition in the document, the text to be rendered, and optionally
the canonical term name. The anchor is converted into a document crossreference in the
Metanorma XML.
...
== Discussion
The source of the concept:[immatk,immature kernel] has not yet been identified.
Allusions to concept:[immatk,unripe kernels,immature kernel] are plentiful in
the literature.
--

If the term is defined in an external document, which has a corresponding bibliographic
anchor, the concept macro gives the bibliographic
anchor of the term definition in that document, the text to be rendered, and optionally
the canonical term name. In other words, the same arguments are used as for the internal
...
== Discussion 
The source of the concept:[iso17301,immature kernel] has not yet been identified.
Allusions to concept:[iso17301,unripe kernels,immature kernel] are plentiful in
the literature.
--
...
== Discussion 
The source of the concept:[iso17301,clause=3.9,immature kernel] has not yet been identified.
Allusions to concept:[iso17301,clause=3.9,unripe kernels,immature kernel] are plentiful in
the literature.
--

If the term is defined in an external termbase, the termbase identifier is given as the
macro target, between the colon and the opening bracket. The identifier within the termbase
is given as the equivalent of the term anchor, as the first argument of the macro. The other
two arguments of the macro are as above, the text to be rendered, and optionally
the canonical term name. There is no expectation that the termbase be included in the bibliography.

[source,asciidoc]
--
== Discussion 
The source of the concept:IEV[171-05-02,immature kernel] has not yet been identified.
Allusions to concept:IEV[171-05-02,unripe kernels,immature kernel] are plentiful in
the literature.
--

...
indicate where the concept is defined.
...
standards documents in Metanorma Semantic XML.

NOTE: The syntax for citing terms has been overhauled for v1.10.0.
This section describes the current syntax
[added in https://github.com/metanorma/metanorma-standoc/releases/tag/v1.10.0].

[source,adoc]
----
{{<<identifier>>,term,display-text,cross-reference,options="..."}}
----

In this full form of the concept macro, *all* arguments are optional, except
for `term`. That means that all of the following are possible:
...
{{<<identifier>>,term,display-text,cross-reference}}
{{<<identifier>>,term,display-text,cross-reference,options="..."}}
----

The `{{<<identifier>>,term,display-text,cross-reference,options=".."}}` markup 
is analogous to the markup of
cross-references in Metanorma AsciiDoc, `\<<anchor,%option,text>>`. It consists of:

* An identifier for the term being cited (optional).
* The term cited, in its canonical form (mandatory).
* The text to be displayed, if it is distinct from the cited term (optional).
If this argument is not provided, the canonical form and the display text are
assumed to be identical.
* Text to display for the crossreference to the concept definition (optional).
If this argument is not provided, the default rendering of the cross-reference
for the current Metanorma flavour is provided.
* Options determining how the concept is to be displayed (may be flavour-specific).

Instances of cited terms are converted into a distinct `concept` element in
Metanorma XML, which includes a cross-reference to the term definition, 
...
{{term,display-text,crossreference-text}}
----
...
and a crossreference to its definition. The default Metanorma rendering would look like:
...
the anchors `\[[term-foo]]` and `\[[term-lor]]` are automatically created and assigned to the terms
'foo` and `lor`.
...
{{<<bibliographic-anchor>>,term,display-text,crossreference-text}}
----
...
NOTE: The Semantic XML preserves the information that the latter term is cited as _unripe kernels_,
but is defined as _immature kernel_. However by default, only the display text is rendered.
...
[source,adoc]
----
{{termbase-identifier:concept-identifier,term}}

// If the display text differs from the cited concept term.
{{termbase-identifier:concept-identifier,term,display-text}}

// If the text of the crossrefernce to the termbase needs to be overridden.
{{termbase-identifier:concept-identifier,term,display-text,crossreference-text}}
----
...
{{IEV:IEV-concept-ID,text,display-term,crossreference-text}}
----

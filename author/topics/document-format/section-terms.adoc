...
* The term is optionally followed by *related terms*, which are marked up with
  `related:[]` commands. Alternative/admitted terms and deprecated terms
  can be marked up with `+alt:[...]+` and `+deprecated:[...]+`, but are treated
  as instances of related terms.
...
In Metanorma, a concept relation is to be made between the current concept and a
target concept. Concept relations are specified in a syntax similar to that for

...
cross-references in Metanorma AsciiDoc, `\<<anchor,text>>`, and consists of:
...
Metanorma XML, which includes a cross-reference to the term definition, and
optionally the canonical form of the term name.
...
NOTE: The terms reference capability relies on automatically created anchor
...
bibliographic anchor for that document.
...
These need to follow the angle brackets around the `bibliographic-anchor`, and
precede the term and/or text.

The syntax is:

[source,adoc]
----
{{<<anchor,locality=X>>,term}}

// Simple example:
{{<<iso639-1,clause=3.1>>,language}}

// If the display text differs from the cited term:
{{<<anchor>>,locality1=X,term,display-text}}

// Multiple localities can be used:
{{<<anchor>>,clause 4.7,table 1,display-text}}
----
...
// The other two arguments of the macro are as above, the canonical term name, and
// optionally the text to be displayed.
...
{{termbase-identifier:concept-identifier,term,text}}
----

NOTE: The termbase does not require a corresponding reference in the
bibliography.

NOTE: Angle brackets are not used for termbase references.

...
By default, Terms and Definitions sections are automatically prefixed with boilerplate,
before any terms ooccur,
indicating the provenance of any definitions (see <<source>>), and, depending on the flavour,
where definitions may be consulted (e.g. for ISO documents, a reference to the ISO Online Browsing Platform
and to the IEC Electropedia). 

Any paragraphs or lists in the AsciiDoc input before the first term are appended to the flavour's defined boilerplate,
in the intermediate XML format [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v1.7.0]. [The
behaviour until that release was for any such text to be overwritten by the flavour's defined boilerplate.] So:
...
under the ISO flavour has the text between the title and the first term definition appended to
the boilerplate required by ISO:
...
If the user wishes to override the boilerplate text with their own boilerplate,
they should provide an initial subclause with the style attribute 
`[.boilerplate]` [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v1.7.0]:
...
[.boilerplate]
=== My boilerplate clause
This is boilerplate that I'm going to use to overwrite the default.
...
The title of the boilerplate clause will be stripped (so you could equally use `=== {blank}`);
the boilerplate is encoded as a subclause, so that its extent can be made unambiguous in initial processing. 
...
If you want to prevent the default boilerplate from appearing, you can do so by providing
an empty boilerplate clause:

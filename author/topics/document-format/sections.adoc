...
and the first section header. The simple foreword syntax should *not* be used
if there are other prefatory clauses expected to precede the foreword;
the Full foreword clause should be used instead.
...
If the "`Foreword`" contains subclauses, or if an explicit AsciiDoc section is preferable
(e.g. there are preceding clauses such as Abstracts), it is encoded as

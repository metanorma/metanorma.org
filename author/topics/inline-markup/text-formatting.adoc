...
As with link:/author/topics/inline-markup/text-formatting#css[CSS declarations],
any font specified as a custom charset font also needs to be passed to Metanorma
in the `:fonts:` document attribute.

For a more deep explanation about encoding custom characters, see
link:/blog/2024-08-12-fonts-special-chars[Encoding and rendering special characters in Metanorma].
...
In addition to these character substitutions, Metanorma converts straight quotes to smart quotes
in postprocessing: see link:/author/ref/document-attributes#smart-quotes[Smart quotes] for the
associated rules. To keep a specific run of text as straight quotes (for example, literal
characters quoted from a specification), use
link:/author/topics/blocks/passthroughs#pass-format-straightquotes[`pass-format:straightquotes[...]`].
AsciiDoc's native plus-delimited inline passthrough (`+...+`) does not prevent this conversion,
because smart quotes are applied in postprocessing after AsciiDoc passthroughs are resolved.

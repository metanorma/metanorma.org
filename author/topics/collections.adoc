...
* DOC: treated as a single document, composed of the individual Metanorma
...
** `documents-inline` (default) indicates that the files should be
...
about the collection; refer to link:/develop/topics/metadata-and-boilerplate.adoc[Metadata and boilerplate].

** `collection-word-coverpage: ...` gives the location of the DOC coverpage for the entire
collection [added in https://github.com/metanorma/metanorma/releases/tag/v1.7.5]. If left out,
or given as `null`, no such coverpage is added.

** `collection-word-intropage: ...` gives the location of the DOC introductory section for the entire
collection [added in https://github.com/metanorma/metanorma/releases/tag/v1.7.5]. As with
HTML and DOC output of standalone documents, this is intended for prefatory material
appearing before user-specified prefatory content: typically boilerplate content, and a placeholder
for the table of contents.
If left out, or given as `null`, no such introductory material is added.

** `document-word-coverpage: ...` gives the location of the DOC coverpage for each individual
document [added in https://github.com/metanorma/metanorma/releases/tag/v1.7.5]. If left out,
or given as `null`, the default cover page that would appear in a standalone document is used.
If given as `""`, no such coverpage is added.

** `document-word-intropage: ...` gives the location of the DOC introductory section for each individual
document [added in https://github.com/metanorma/metanorma/releases/tag/v1.7.5]. If left out,
or given as `null`, the default introductory section that would appear in a standalone document is used.
If given as `""`, no such introductory section is added.
...
* `format`, specifying the possible output formats for the collection as a list,
as an alternative to the `-f` argument of `metanorma collection`.
Allowed values are:

** `xml`
** `presentation` (for Presentation XML, added automatically if any of the following formats is specified)
** `html`
** `doc`
** `pdf`
...
** If `sectionsplit` is `true`, the HTML output for the specified file
is arranged as one HTML file per clause, with an index page created for the overall
document. The index page for the entire document links to the index page for the `sectionsplit` document.

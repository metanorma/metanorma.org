...
[[iso-iec-directives]]ISO/IEC Directives::
`ISO/IEC DIR {n}`, `ISO/IEC DIR {n} + {flavor} SUP`, or `ISO/IEC DIR {flavor} SUP`. `n` - is a number of directive, `flavor` - is `ISO` or `IEC`.
+
Following referenses are supported at this time:
+
...
- `ISO/IEC DIR 2 ISO` Principes et règles de structure et de rédaction des documents
- `ISO/IEC DIR IEC SUP` Procedures specific to IEC
- `ISO/IEC DIR JTC 1 SUP` Procedures specific to JTC 1
+
NOTE: This function depends on the <<Static cache>>. Documents in the <<Static cache>> could have differences from the online version.
...
The Static cache is distributed with the `relaton` gem and always enabled. It contains some documents, like <<ISO/IEC Directives>>, which can't be fetched from websites.

To add a new document to the Static cache please make an issue on https://github.com/relaton/relaton/issues.

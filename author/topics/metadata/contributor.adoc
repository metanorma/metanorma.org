...
Contributors are specified in document metadata using document attributes.
...
Contributor groups of organizations, i.e. editorial committees, are specified
using document attributes.

Different Metanorma flavors utilize different types of group semantics.

.Example of SDO-specific group contributor for ISO and IEC
====
ISO and IEC use group semantics composing of technical committees,
...
For different flavors, the accepted types of committee are as follows:
...
`{group}-type`:: the type of the group, as an abbreviation; the identifier of
the group is assumed to be type + number, e.g. _TC 2_

`{group}-agency`:: the agency that the group lies under, in case of multiple groups

`{group}-agency-abbr`:: the abbreviation of the agency that the group lies
under, in case of multiple groups

`{group}_logo`:: the image path to the logo of group.

...
Both prefatory and final content are entered as Asciidoc, and are compiled
as a document in the same flavor as the collection, with `bibdata`
(bibliographic metadata) taken from the collection. This bibdata is merged
with any bibdata that is added in processing the Asciidoc; that for instance
allows markup of document history to be passed into a collection as metadata

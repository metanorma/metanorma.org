...
`base`::: Sets the numeric base (radix) used to render both the integer and fractional parts 
of the number [added in https://github.com/metanorma/isodoc/releases/tag/v3.5.1]. 
...
This is applied after digit grouping. `nil` or an empty string can be used to omit the postfix.

`hex_capital`::: When `true` and `base` is `16`, all characters in the range `a-f` in the converted output
are rendered using uppercase letters of the number [added in https://github.com/metanorma/isodoc/releases/tag/v3.5.1]. 
This includes both hexadecimal digits and any separators (such as `decimal`, `group`, `fraction_group`) that happen
to be letters in the `a-f` range. The `base_prefix` and `base_postfix` values are never modified. It has no effect for other bases.

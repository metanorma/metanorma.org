...
The option is automatically populated if a `+` appears in front of the number entered:
...
Inline stem macros ( `\stem:[ ]` ) do not support attributes, so specifying
number formatting on inline stem macros is currently supported 

* either
through the universal application of `:number-presentation-formula:`,
* or by providing the number formatting as an attribute string delimited by `%`
(as with the extensions to Asciidoc cross-references in 
Metanorma) [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.9.7].
If `%` needs to appear as a number formatting attribute value, encode it as the
XML entity `\&#x25;`.

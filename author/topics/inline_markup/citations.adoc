...
Custom words can be substituted for the predefined conjoining verbs by appending
a colon, following by the desired verb, which remains mandatory to determine
semantics [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.3.0].

[example]
The rendering of the `to!` conjunction of "_to_" can be replaced with
"_through_" by specifying `to:through!`, which indicates that the intended
semantics is still that of "_to_", but the word _through_ should be used in
rendering instead.

...
If the `:document-scheme:` is not provided, the scheme to apply is inferred based on the `:copyright-year:` document attribute
(without allowing for the month in which transition was made from one scheme to another -- which in any case was
not always a clean transition historically) [added in https://github.com/metanorma/metanorma-iso/releases/tag/v2.7.6].
If neither of those attributes is present, the current default is applied, `2024`.

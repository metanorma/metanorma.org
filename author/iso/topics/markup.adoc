...
Note that the `start` attribute will be *ignored* in Word output for stages
between 00 and 39, because Word output for those stages involves autonumbering of
lists, for which Metanorma does not yet support `start` in Word. Stages from 40 onward
insert explicit numbers in lists in Word, and so do not rely on Word autonumbering.
...
Editorial notes can be added to an ISO document, to differentiate feedback from ISO editors to
the readers of a circulated draft, from feedback to the authors or editors of a draft.
Editorial notes can be marked up as follows [added in https://github.com/metanorma/metanorma-iso/releases/tag/v2.1.2]:

...
The following document relations have been introduced for compatibility with ISO STS [added in https://github.com/metanorma/metanorma-iso/releases/tag/v1.10.4]. They are populated as with other document relation attributes;
see link:/author/ref/document-attributes/#document-relations[Document relations].

`revises`:: STS `revises`
`replaces`:: STS `replaces`
`amends`:: STS `amends`
`corrects`:: STS `corrects` 
`informatively-cited-in`:: STS `informativelyReferencedBy`
`informatively-cites`:: STS `informativelyReferences`
`normatively-cited-in`:: STS `normativelyReferencedBy`
`normatively-cites`:: STS `normativelyReferences`
`identical-adopted-from`:: STS `isIdenticalNationalStandardOf`
`modified-adopted-from`:: STS `isModifiedNationalStandardOf`
`successor-of`:: STS `isProgressionOf`
`manifestation-of`:: STS `isPublishedFormatOf`
`related-directive`:: STS `relatedDirective`
`related-mandate`:: STS `relatedMandate`
`supersedes`:: STS  `supersedes`
`annotation-of`:: STS  `commentOn`
`related`::  STS  `""` (empty value)

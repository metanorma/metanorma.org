...
`:sort-biblio:`::
Sort the bibliographies in the document according to criteria set by the SDO
 [added in https://github.com/metanorma/metanorma-standoc/releases/tag/3.3.0]. The default value for
...
`:publisher_logo{_i}:`:: The logo of the publisher, specified as an image file; the numbers in the attribute
align to the `:publisher{_i}:` attributes [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.7.0].
...
that person should be treated as a personal contributor (`:surname{_i}:`, `:affiliation{_i}:`, etc.),
with a `:role:` attribute of `enabler`.

`:sponsor_logo{_i}:`:: The logo of the sponsoring organization, specified as an image file; the numbers in the attribute
align to the `:sponsor{_i}:` attributes [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.7.0].
...
The name of a relevant editorial group. The document attribute name for editorial groups varies by flavor,
and there can be multiple types of editorial group, each with its own document attribute;
see link:/author/topics/metadata/contributor#committee-groups[Contributor metadata: Groups] for the value
for each flavor. In the following list, `technical-committee` can be replaced by each of the allowed
editorial group labels; for example, if `workgroup` is an editorial group label, then `:workgroup-label:` and
`:workgroup-type:` are document attributes, instead of `:technical-committee-label:` and
...
The type of the editorial group. Typically this is used as the abbreviation prefixed to the editoriall group number;
e.g. `:technical-committee-number: 2`, `:technical-committee-type: TC`, giving _TC 2_.

`:technical-committee-agency{_i}:`:: The agency that the editorial group lies under, if it is other than the home agency
...
`:technical-committee-agency-abbr{_i}:`:: The abbreviation of the agency that the editorial group
lies under [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.1.4].
In most flavors, the abbreviation of the SDO is already available for lookup in Metanorma, and users
need not provide this attribute; e.g. under metanorma-iso, `:technical-committee-agency:` can be specified as `ISO`,
rather than needing to specify both `:technical-committee-agency: International Standardization Organization` and
...
`:committee-types:`:: Comma-delimited list of committee types to be recognised 
for the document  [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.3.0].
Overrides the preset commitee types set for the flavor.
...
align to the `:authorizer{_i}:` attributes   [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.8.3].
...
other locations in the document [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.2.6].
...
`:erefstyle:`::
Override the default rendering of cross-references to bibliographic entries
...
`:originstyle:`::
Override the default rendering of source references to bibliographic entries
 [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.3.0].
The legal values for such override are drawn from link:/author/topics/inline_markup/citations/#citation-style-types[General citation style types].

...
All the following attributes relate to PDF protection from copying [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v1.11.3].

`:pdf-encrypt:`::
Specify encryption of PDF output. (`true, false`, default `false`.)

`:pdf-encryption-length:`::
Specify encryption strength. (`128, 256`, corresponding to 128-bit or 256-bit AES keys.)
...
Allow content to be copy-pasted from the PDF document. (`true, false`, default `true`.)

`:pdf-allow-edit-content:`::
Allow content of the PDF document to be edited. (`true, false`, default `true`.)

`:pdf-allow-assemble-document:`::
Allow inserting, deleting, or rotating pages in the PDF document. (`true, false`, default `true`.)

`:pdf-allow-edit-annotations:`::
Allow annotations and signatures to be added to the PDF document. (`true, false`, default `true`.)

`:pdf-allow-print:`::
Allow PDF document to be printed, physically or to file. (`true, false`, default `true`.)

`:pdf-allow-print-hq:`::
Allow PDF document to be printed in high quality. (`true, false`, default `true`.)

`:pdf-allow-fill-in-forms:`::
Allow forms to be filled in the PDF document. (`true, false`, default `true`.)

`:pdf-allow-access-content:`::
Allow text and graphics extraction from the PDF document for accessibility purposes. (`true, false`, default `true`.)

`:pdf-encrypt-metadata:`::
Specify encryption of the metadata stream. (`true, false`, default `false`.)

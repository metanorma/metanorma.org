...
`:document-scheme:`::
Iteration of document content arrangement that Metanorma will enforce for this document. Flavour-specific.
Metanorma inserts clauses with predetermined text, and orders clauses, in accordance with the style prescribed
by the SDO; if those styles are updated, this attribute indicates to Metanorma which iteration of the prescription
to enforce  [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v2.1.6]. As of this writing,
only implemented in the IEEE flavour.

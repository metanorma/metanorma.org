...
`:author_{i}:`:: The organization authoring the standard [added in https://github.com/metanorma/metanorma-standoc/releases/tag/v3.1.5].
This is normally identical to the agency publishing the standard, as specified in `:publisher:` or populated by default,
in which case `:author:` should not be provided. It is only provided in rare cases
where a standard is authored or co-authored with a distinct agency, but only the default agency is credited as the publisher. More than one
agency can be specified as the author, as with `publisher`; the first agency specified as author is expected to be the publishing agency.
Personal authors are instead specified through `:fullname:` or `:surname:` and `:givenname:`.

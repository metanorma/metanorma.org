...
It is possible to specify replacement predefined text in the document preface,
using `:boilerplate-authority:`, as documented in link:/develop/topics/metadata-and-boilerplate/[Metadata and boilerplate].
In addition, if a document is published by two or more SDOs, it is possible to specify additional
copyright notices for the other SDOs involved, through `copyright-statement-append` clauses
in the custom boilerplate text file, as is also specified there.
...
For bibliographic citations rendered with bracketed numbers, such as _[3]_,
Metanorma follows the ISO House style and ISO/IEC DIR 2 in its rendering,
in differentiating between sentence and non-sentence 
citations [added in https://github.com/metanorma/metanorma-iso/releases/tag/v3.1.5].

* By default, citations are considered to be non-sentence, and have the word "Reference"
prefixed to them. This word is translated in languages other than English, and pluralised
if it precedes a grouping of more than one reference.
* A citation is considered to be in-sentence if it is given `style=superscript%` as its style.
If that is the case, it is rendered instead as a superscript (following ISO House style),
and the word "Reference" is not prefixed.
** Metanorma will remove any space between the preceding sentence word and the superscript citation.
** Metanorma will *not* move the superscript citation before any punctuation, as the risk of
error is too great. That is the responsibility of the author.
    
So the following markup of references `ref1` anf `ref2`, which are to be cited as _[1]_ and _[2]_:

[source,asciidoc]
----
Sentence <<ref1>>.

Sentence <<ref1,style=superscript%>>.

Sentence <<ref1;and!ref2>>.

Sentence <<ref1;and!ref2,style=superscript%>>.

Sentence <<ref1,clause=3>>.

Sentence <<ref1,style=superscript%clause=3>>.
----

will be rendered as follows, with no intervention needed from the user:
...
Sentence References [1] and [2].

Sentence^[1]^ ^and^ ^[2]^.

Sentence Reference [1], Clause 3.
...
link:/author/topics/inline-markup/changes/[Machine-readable changes].

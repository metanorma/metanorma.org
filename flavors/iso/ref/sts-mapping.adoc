...
[source,xml]
----
<ext-link ext-link-type="uri" pass:[xlink:href]="http://www.iso.org/directives"> www.iso.org/directives</ext-link>
----
====
...
a|
a|
`ext-link/@pass:[xlink:href]`

[example%unnumbered]
====
[source,xml]
----
<ext-link xlink:type="simple" pass:[xlink:href]="http://bsigroup.com/standards"> bsigroup.com/standards</ext-link>
----
====
...
pass:[xlink:href]="30238432_FigA01a">
...
a|
a|
`inline-graphic \| graphic/@pass:[xlink:href] and not processing-instruction('isoimg-id')`

NOTE: if there isn't `processing-instruction('isoimg-id'`), then `@pass:[xlink:href]` contains image file name.
| *`>`*
a|
`image/@src` +
`string in base64 encoding OR` +
`imagedir path + @pass:[xlink:href] + .png(optional, if @pass:[xlink:href] doesn't have file extension)` | `image:: ...`
a|
`graphic/processing-instruction('isoimg-id')`

NOTE: if there is this PI, then it contains image file name (but not `@pass:[xlink:href]`)

NOTE: `graphic/@pass:[xlink:href]` +
*conversion gap* +

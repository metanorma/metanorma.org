...
[[cli-extensions]]
`-x, --extensions FORMATS`:: Comma-separated list of output formats to generate
(e.g. `html,rxl`), restricting output to those formats only.
Overrides the manifest `extensions:` key (see <<manifest-extensions>>) and any
per-collection `format:`.
Defaults to all formats the document's flavor supports.
...
<7> [[manifest-extensions]] `metanorma.extensions`:  Restrict the output formats generated for all sources in this run (e.g. `html`, `rxl`), overriding any per-collection `format:`. Omit to generate every format the flavor supports. The CLI `--extensions` / `-x` flag (see <<cli-extensions>>) takes precedence over this key.
...
* `docstatus.stage.abbreviation` - Abbreviation for the document status
...
To publish compiled documents as GitHub Releases and aggregate them into an
organization portal, see

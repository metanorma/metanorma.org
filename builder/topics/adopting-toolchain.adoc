...
* Clone the `metanorma-acme` gem: https://github.com/metanorma/metanorma-acme (if you will override the use of config files for metadata), or `metanorma-rsd`: https://github.com/metanorma/metanorma-rsd (if you are happy to keep using a config file).
* Change the root tag and namespace (`XML_ROOT_TAG, XML_NAMESPACE` in `.../lib/asciidoctor/{flavour}/converter.rb`) to a namespace specific to your organization's document standard (or use the configuration YAML file as documented for Simple Adoption, channging `metanorma.yml.example` to `metanorma.yml`).
* Change any references to `acme` or `Acme` (or `rsd` or `RSD`) in the gem to your organization's document standard.
* Change the styling of the document outputs (`.../lib/isodoc/{flavour}/html`).

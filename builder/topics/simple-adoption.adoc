...
`logo_path`:: path to a single logo file, used for metanorma metadata
`logo_paths`:: list of paths of multiple logo files, used for metanorma metadata, and treated as an array (e.g. `{{ logo_paths[1] }}` [added in https://github.com/metanorma/metanorma-generic/releases/tag/v1.4.8].
...
`metadata_extensions`:: A list of single-value fields to add to `bibdata/ext` for the document, as metadata; they will be populated through document attributes with the same name [added in https://github.com/metanorma/metanorma-generic/releases/tag/v1.4.7].

...
+
NOTE:: *.scss stylesheets (which can include variables like `html_bodyfont` and `html_headerfont`) require the `scss` gem to be compiled into *.scss; you will need to add `gem "sassc"` to your Gemfile. (Alternatively, used pure .css stylesheets instead, with any `$` variables filled in with values.)
...
For example, for Metanorma Ribose, it is `metanorma-ribose`.
...
Metanorma::Generic.configure do |config|
  ...
  config.standardstylesheet = File.join(isodoc_rsd_html_folder, 'ribose.scss')
